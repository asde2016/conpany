                ...
                <div class="board_container">
                    ...
                    <div class="table_wrapper">
                            ...
                            <tbody>
                                <tr @click="goToDetail(board.b_sno)" :key="i" v-for="(board,i) in boardListInPage">
                                    ...
                                    <td colspan="4">
                                        ...
                                        <a v-else @click="goToDetail(board.b_sno)" href="#">{{board.b_title}}<i v-if="(board.board_file_cnt > 0)" class="fas fa-file-alt" aria-hidden="true"></i></a>
                                    </td>
                                    <td colspan="2">
                        ...
                        <div class="edit clearfix">
                            <router-link to="/department/create" href="#">글작성</router-link>
                        </div>
                        <nav id="pagination"> 
                            <ul>
...
export default {
    ...
    methods: {
        async getBoardList() {
            ...
            let dSno = this.$store.state.user.m_sno == 0
                     ? this.setIncludeDepartment()
                     : this.$store.state.user.d_sno;
            this.boardList = await this.$api("/api/boardList",{param:[this.typeSno, dSno]});
        ...
        goToDetail(b_sno) {
            this.$router.push({path: '/department/detail', query:{b_sno: b_sno}});
        },
        pageActive(pageNum) {
