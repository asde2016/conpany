                ...
                <div class="board_container">
                    <div class="title clearfix">
                        ...
                        <div class="form">
                            ...
                            <input @click="goSearch(1)" type="button" value="검색">
                        </div>
                    </div>
                    <div class="table_wrapper">
                        <table class="table notice list">
                            ...
                            <tbody>
                                <tr @click="goToDetail(board.b_sno)" :key="i" v-for="(board,i) in boardListInPage">
                                    ...
                                    <td colspan="4">
                                        <a v-if="board.new !== 0" href="#"><span class="new">NEW</span>{{board.b_title}}<i v-if="(board.board_file_cnt > 0)" class="fas fa-file-alt" aria-hidden="true"></i></a>
                                        <a v-else href="#">{{board.b_title}}<i v-if="(board.board_file_cnt > 0)" class="fas fa-file-alt" aria-hidden="true"></i></a>
                                    </td>
                                    <td colspan="2">
                        ...
                        <div class="edit clearfix">
                            <router-link v-if="this.$store.state.user != null && this.$store.state.user.m_sno == 0" to="/notice/create" href="#">글작성</router-link>
                        </div>
                        <nav id="pagination"> 
                            <ul>
...
export default {
    ...
    methods: {
        async getBoardList() {
            this.boardList = await this.$api("/api/boardList",{param:[this.typeSno, 0]});
            this.totalPage = Math.floor(this.boardList.length / this.perPage) > 0
                           ? Math.floor(this.boardList.length / this.perPage) + 1
                           : 1;
        },
        goToDetail(b_sno) {
            this.$router.push({path: '/notice/detail', query:{b_sno: b_sno}});
        },
        pageActive(pageNum) {
